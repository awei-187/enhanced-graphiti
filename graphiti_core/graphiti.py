...
# Import caches
from graphiti_core.cache import SemanticCache, SubgraphCache
...
class Graphiti:
    def __init__(
        ...
        cache_index_type: str = 'auto',
        cache_hnsw_m: int = 32,
        cache_hnsw_ef_search: int = 64,
        cache_hnsw_ef_construction: int = 200,
        # ========== Subgraph Cache Configuration ==========
        enable_subgraph_cache: bool = False,
        subgraph_cache_ttl_seconds: float = 300.0,
        subgraph_cache_max_entries: int = 100,
        subgraph_cache_min_top1_score: float = 0.75,
        subgraph_cache_max_candidates: int = 50,
    ):
        """
        ...
        enable_cache : bool, optional
            Whether to enable semantic caching for search queries. Defaults to True.
        cache_similarity_threshold : float, optional
            Minimum cosine similarity score for cache hit (0.0-1.0). Defaults to 0.9.
        cache_ttl_seconds : float, optional
            Time-to-live for cache entries in seconds. Defaults to 300 (5 minutes).
        cache_max_entries : int, optional
            Maximum number of entries in the cache. Defaults to 1000.
        cache_index_type : str, optional
            Index type for semantic cache: auto, faiss_exact, faiss_hnsw, or numpy.
        cache_hnsw_m : int, optional
            HNSW graph degree (used when cache_index_type is faiss_hnsw).
        cache_hnsw_ef_search : int, optional
            HNSW search parameter (used when cache_index_type is faiss_hnsw).
        cache_hnsw_ef_construction : int, optional
            HNSW build parameter (used when cache_index_type is faiss_hnsw).
        enable_subgraph_cache : bool, optional
            Whether to enable subgraph caching for search queries. Defaults to False.
        subgraph_cache_ttl_seconds : float, optional
            Time-to-live for subgraph cache entries in seconds. Defaults to 300.
        subgraph_cache_max_entries : int, optional
            Maximum number of subgraph cache entries. Defaults to 100.
        subgraph_cache_min_top1_score : float, optional
            Minimum normalized top-1 score required to use a cached subgraph result.
            Defaults to 0.75.
        subgraph_cache_max_candidates : int, optional
            Maximum local candidates to rerank from a cached subgraph. Defaults to 50.
        ...
        if enable_cache:
            self.cache = SemanticCache(
                ...
                hnsw_m=cache_hnsw_m,
                hnsw_ef_search=cache_hnsw_ef_search,
                hnsw_ef_construction=cache_hnsw_ef_construction,
                enabled=True,
            )
            logger.info(
                f'Semantic cache enabled: threshold={cache_similarity_threshold}, '
                f'ttl={cache_ttl_seconds}s, max_entries={cache_max_entries}, '
                f'index={self.cache.index_type}'
            )
        else:
            self.cache = None
        ...
        self.enable_subgraph_cache = enable_subgraph_cache
        if enable_subgraph_cache:
            self.subgraph_cache = SubgraphCache(
                ttl_seconds=subgraph_cache_ttl_seconds,
                max_entries=subgraph_cache_max_entries,
                min_top1_score=subgraph_cache_min_top1_score,
                max_candidates=subgraph_cache_max_candidates,
                enabled=True,
            )
            logger.info(
                f'Subgraph cache enabled: threshold={subgraph_cache_min_top1_score}, '
                f'ttl={subgraph_cache_ttl_seconds}s, max_entries={subgraph_cache_max_entries}'
            )
        else:
            self.subgraph_cache = None
            logger.info('Subgraph cache disabled')
    ...
    async def add_episode(
        ...
        with self.tracer.start_span('add_episode') as span:
            try:
                ...
                # When new entities are added/updated, invalidate related cache entries
                if hydrated_nodes:
                    updated_entity_names = {node.name for node in hydrated_nodes}
                    if self.cache is not None:
                        invalidated_count = self.cache.invalidate_by_entities(updated_entity_names)
                        if invalidated_count > 0:
                            logger.info(
                                f'Cache invalidated {invalidated_count} entries for entities: '
                                f'{list(updated_entity_names)[:5]}...'
                            )
                    if self.subgraph_cache is not None:
                        subgraph_invalidated_count = self.subgraph_cache.invalidate_by_entities(
                            updated_entity_names
                        )
                        if subgraph_invalidated_count > 0:
                            logger.info(
                                f'Subgraph cache invalidated {subgraph_invalidated_count} '
                                f'entries for entities: {list(updated_entity_names)[:5]}...'
                            )
    ...
    async def add_episode_bulk(
        ...
        with self.tracer.start_span('add_episode_bulk') as bulk_span:
            ...
            try:
                ...
                # ========== Event-Driven Cache Invalidation ==========
                # When new entities are added/updated in bulk, invalidate related cache entries
                if final_hydrated_nodes:
                    updated_entity_names = {node.name for node in final_hydrated_nodes}
                    if self.cache is not None:
                        invalidated_count = self.cache.invalidate_by_entities(updated_entity_names)
                        if invalidated_count > 0:
                            logger.info(
                                f'Cache invalidated {invalidated_count} entries for {len(updated_entity_names)} entities'
                            )
                    if self.subgraph_cache is not None:
                        subgraph_invalidated_count = self.subgraph_cache.invalidate_by_entities(
                            updated_entity_names
                        ...
                        if subgraph_invalidated_count > 0:
                            logger.info(
                                f'Subgraph cache invalidated {subgraph_invalidated_count} '
                                f'entries for {len(updated_entity_names)} entities'
                            )
    ...
    async def search(
        ...
        edges = (
            await search(
                ...
                use_subgraph_cache=self.enable_subgraph_cache,
            )
        ).edges
    ...
    def get_cache_stats(self) -> dict | None:
        ...
        -------
        dict | None
            Dictionary containing cache statistics (hit_rate, latency, etc.),
            or None if caching is disabled.
        """
        if self.cache is None:
            return None
        stats = self.cache.get_stats().to_dict()
        stats['index_type'] = self.cache.index_type
        return stats

    def get_subgraph_cache_stats(self) -> dict | None:
        """
        ...
        -------
        dict | None
            Dictionary containing subgraph cache statistics, or None if disabled.
        """
        if self.subgraph_cache is None:
            return None
        return self.subgraph_cache.get_stats().to_dict()

    def clear_cache(self) -> None:
        """Clear all entries from enabled search caches."""
        if self.cache is not None:
            self.cache.clear()
    ...
    def invalidate_cache_for_entities(self, entity_names: set[str]) -> int:
        """
        Manually invalidate cache entries related to specific entities.

        Parameters
        ----------
        entity_names : set[str]
            Set of entity names to invalidate.
        ...
        invalidated_count = 0
        if self.cache is not None:
            invalidated_count += self.cache.invalidate_by_entities(entity_names)
        if self.subgraph_cache is not None:
            invalidated_count += self.subgraph_cache.invalidate_by_entities(entity_names)
